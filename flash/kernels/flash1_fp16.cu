// source: https://github.com/tspeterkim/flash-attention-minimal/blob/tensorcore/flash.cu
...
torch::Tensor forward(torch::Tensor Q, torch::Tensor K, torch::Tensor V) {
    bool is_fp16 = (Q.scalar_type() == torch::kFloat16);
    if (is_fp16) {
    ...
    if (is_fp16) return O.to(torch::kFloat16);
    return O;
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("forward", &forward, "Tiled attention forward");
}
